...
One-line titles — pull request titles and commit messages — are concise English
following [Conventional Commits](https://www.conventionalcommits.org/), because
they flow through tooling.
A change is titled once: the pull request title is written as the commit message
for the whole change, so that it can land on `main` unedited.
...
(`feat(spec-drafting): add a prototype planning phase`); omit it otherwise
(`docs: update the skill placement guide`).
Mark the type with `!` only when a skill is renamed or removed, since that breaks
