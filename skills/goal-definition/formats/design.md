...
- `Related Files`: bullet points only. List created companion handoff files that own separate decision areas, or write `- None` when there are none. Keep companion-owned decisions out of `design.md`.
...
- `Completion Conditions`: bullet points only. Describe the observable state that should be true when the work is complete, including behavior whose presence is central to the design; if a later agent followed them until all are true, the target outcome should be achieved. Do not describe how to verify those conditions, turn them into a task list, or add a separate testing section.
