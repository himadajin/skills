...
Use this format for PHASE2 only. `design.md` must be self-contained: if it is handed to a context-reset coding agent without `desire.md`, the agent should still have enough context to implement the work.
...
- `Purpose`: state the desired outcome in implementation-facing terms, not merely the initial request.
