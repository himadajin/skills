...
name: bash-scripting
description: >-
  Create a single-file Bash command-line script
...
- Resolve required external commands during the interview, verifying they
  exist with `command -v`.
- Do not hand-roll parsers for structured data formats in Bash; use an agreed
  purpose-built command, or propose switching to the python-scripting skill
  when no suitable command exists.
