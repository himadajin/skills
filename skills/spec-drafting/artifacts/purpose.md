...
The draft lives only in conversation and is never written to a file; its
agreements flow into the first draft of `spec.md`.

## Sections

Use `Purpose`, `Context`, and `Direction`. Exclude `Related Files`,
...
might be realized. The closing action is to keep the brief in conversation and
move to `spec.md`.
